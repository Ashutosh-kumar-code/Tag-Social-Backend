...
exports.isBlock = async (req, res) => {
  try {
    // Ensure userId is provided and it's not empty
    const { userId } = req.query;
    if (!userId) {
      return res.status(400).json({ status: false, message: "userId is required." });
    }

    // Split the userId by commas to handle multiple IDs
    const userIds = userId.split(",");

    // Validate each userId to ensure it's a valid ObjectId format
    ...
    // Find users by their ObjectId
    const users = await User.find({ _id: { $in: userIds } });

    // If not all users are found, return an error
    if (users.length !== userIds.length) {
      return res.status(404).json({ status: false, message: "Oops! Not all users found." });
    }

    // Toggle the block status for each user
    for (const user of users) {
      user.isBlock = !user.isBlock;  // Toggle block status
      await user.save();  // Save the updated user
    }

    return res.status(200).json({ status: true, message: "User block status updated successfully.", users });
